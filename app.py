...
def tab_pmc(da):
    """
    PMC — usa icu_training_load para CTL/ATL/FTLM (escala Intervals.icu).
    Load bars (TRIMP) = session_rpe = moving_time_min × RPE.
    Histórico completo 730d via session_state.
    """
    ...
    # ── CTL/ATL: icu_training_load (1ª opção) ou session_rpe (fallback) ──
    if 'icu_training_load' in df.columns and df['icu_training_load'].notna().sum() > 10:
        ...
        _load_metrica_ctl = "icu_training_load (Intervals.icu)"
        ...
        _load_metrica_ctl = "session_rpe (fallback — sem icu_training_load)"
    else:
        st.warning("Sem icu_training_load nem RPE para calcular CTL/ATL."); return

    # ── Load bars: TRIMP = session_rpe (independente do CTL) ──
    _rpe2 = pd.to_numeric(df.get('rpe', pd.Series(dtype=float)), errors='coerce')
    _mt   = pd.to_numeric(df.get('moving_time', pd.Series(dtype=float)), errors='coerce')
    if _rpe2.notna().sum() > 5:
        df['trimp_val'] = (_mt / 60) * _rpe2.fillna(_rpe2.median())
    else:
        df['trimp_val'] = df['icu_tl']

    # ── Série diária para CTL/ATL ──
    ...
    st.caption(f"CTL/ATL/FTLM: **{_load_metrica_ctl}** | "
               f"Barras de Load: **TRIMP (session_rpe)** | Histórico: {len(ld)} dias")

    # ── Controlos ──
    col1, col2, col3 = st.columns(3)
    dias_exib_opts = {"30 dias": 30, "60 dias": 60, "90 dias": 90,
                      "180 dias": 180, "1 ano": 365, "Todo histórico": len(ld)}
    dias_exib = dias_exib_opts[col1.selectbox("Período exibido",
                list(dias_exib_opts.keys()), index=2)]
    ...
    ax_pmc.plot(ld_plot['Data'], ld_plot['CTL'],
                label='CTL (Fitness)', color=CORES['azul'], linewidth=2.5)
    ax_pmc.plot(ld_plot['Data'], ld_plot['ATL'],
                label='ATL (Fadiga)',  color=CORES['vermelho'], linewidth=2.5)
    ...
    for tipo in tipos_ord:
        ...
        vals = merged['trimp_val'].values
        ax_load.bar(ld_plot['Data'], vals, bottom=bot, color=get_cor(tipo),
                    alpha=0.85, width=0.8, label=tipo,
    ...
    # ── GRÁFICO 2: KM stacked por modalidade (excl. WeightTraining) ──
    if 'distance' in df.columns and df['distance'].notna().any():
        st.subheader("📏 Distância (KM) por modalidade")
        ...
        if len(df_km) > 0:
            ...
            if len(km_sem) > 0 and tipos_km:
                fig2, ax_km = plt.subplots(figsize=(16, 5))
                semanas = sorted(km_sem['Semana'].unique())
                bot_km = np.zeros(len(semanas))
                for tipo in tipos_km:
                    vals_km = np.array([
                        float(km_sem[(km_sem['Semana']==s) &
                                     (km_sem['type']==tipo)]['km'].values[0])
                        if len(km_sem[(km_sem['Semana']==s) &
                                      (km_sem['type']==tipo)]) > 0 else 0
                        for s in semanas])
                    ax_km.bar(semanas, vals_km, bottom=bot_km,
                              color=get_cor(tipo), alpha=0.85, width=5,
                              label=tipo, edgecolor='white', linewidth=0.3)
                    bot_km += vals_km
                ax_km.set_ylabel('Distância (km)', fontweight='bold')
                ax_km.set_title('Distância semanal por modalidade (KM)',
                                fontsize=13, fontweight='bold')
                ax_km.legend(fontsize=9, ncol=len(tipos_km))
                ax_km.grid(True, alpha=0.2, axis='y')
                plt.xticks(rotation=45); plt.tight_layout()
                st.pyplot(fig2); plt.close()

                # Tabela KM por quartil
                st.markdown("**Distância (KM) por modalidade — estatísticas e quartis**")
                quartil_rows = []
                for tipo in tipos_km:
                    km_t = km_sem[km_sem['type'] == tipo]['km']
                    if len(km_t) < 2: continue
                    quartil_rows.append({
                        'Modalidade':  tipo,
                        'N semanas':   len(km_t),
                        'Total KM':    f"{km_t.sum():.0f}",
                        'Media/sem':   f"{km_t.mean():.1f}",
                        'Q1 (25%)':    f"{km_t.quantile(0.25):.1f}",
                        'Mediana':     f"{km_t.median():.1f}",
                        'Q3 (75%)':    f"{km_t.quantile(0.75):.1f}",
                        'Maximo':      f"{km_t.max():.1f}",
                    })
                if quartil_rows:
                    st.dataframe(pd.DataFrame(quartil_rows),
