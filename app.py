...
MAPA_TRAINING = {
    ...
    'moving_time': ['moving_time', 'duration'],
    'power_avg': ['icu_average_watts', 'average_watts'],
...
def carregar_atividades(days_back):
    ...
    try:
        ...
        for var, possiveis in MAPA_TRAINING.items():
            col = detectar_coluna(df, possiveis)
            if col:
                df[var] = df[col] if var in ['type','name','date'] else df[col].apply(br_to_float)
