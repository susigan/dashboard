...
def tab_zones(da, mods_sel):
    ...
    # icu_rpe primeiro — é a coluna correcta do Intervals.icu
    rpe_col  = next((c for c in ['icu_rpe','rpe','RPE'] if c in df.columns), None)
    ...
    _CORES_ANOS = ['#1a73e8','#e8710a','#33a853','#d93025','#9c27b0','#00bcd4','#ff9800']
    ...
    # ── Botões de ano — multi-selecção para comparação ──────────────────
    anos = sorted(df['ano'].unique())

    if 'zones_anos_sel' not in st.session_state:
        st.session_state['zones_anos_sel'] = [anos[-1]]
    # Garantir que anos seleccionados ainda existem nos dados
    st.session_state['zones_anos_sel'] = [
        a for a in st.session_state['zones_anos_sel'] if a in anos
    ]
    if not st.session_state['zones_anos_sel']:
        st.session_state['zones_anos_sel'] = [anos[-1]]

    st.markdown("**📅 Seleccionar ano(s) — múltiplos para comparar:**")
    _btn_cols = st.columns(len(anos))
    for _i, _ano in enumerate(anos):
        _is_active = _ano in st.session_state['zones_anos_sel']
        if _btn_cols[_i].button(
            str(_ano),
            key=f"zones_btn_{_ano}",
            help=f"{'Deseleccionar' if _is_active else 'Seleccionar'} {_ano}",
            use_container_width=True,
            type="primary" if _is_active else "secondary"
        ):
            if _is_active and len(st.session_state['zones_anos_sel']) > 1:
                st.session_state['zones_anos_sel'].remove(_ano)
            elif not _is_active:
                st.session_state['zones_anos_sel'].append(_ano)
            st.rerun()

    anos_sel = sorted(st.session_state['zones_anos_sel'])
    _modo_comparacao = len(anos_sel) > 1

    if _modo_comparacao:
        st.info(f"📊 **Modo comparação:** {' vs '.join(str(a) for a in anos_sel)}")

    df_ano  = df[df['ano'].isin(anos_sel)].copy()
    ano_sel = ' vs '.join(str(a) for a in anos_sel)

    # ════════════════════════════════════════════════════════════════════
    # GRÁFICOS — ano(s) seleccionado(s)
    # ════════════════════════════════════════════════════════════════════
        ...
        if not _modo_comparacao:
            ...
            for zona, cor in CORES_HR.items():
                ...
                fig_hr_mod.add_trace(go.Bar(
                    x=pt_hr['type'], y=vals, name=zona, marker_color=cor,
                    text=[f"{v:.0f}%" for v in vals], textposition='inside',
                    textfont=dict(color='white', size=11, family='Arial'),
                    hovertemplate=f'{zona}: <b>%{{y:.1f}}%</b><extra></extra>'))
            fig_hr_mod.update_layout(**PW, barmode='stack',
                title=dict(text=f'❤️ HR Zones por Modalidade — {ano_sel}', font=dict(color='#222', size=13)),
                height=380, legend=LEGEND,
                xaxis=dict(title='', tickfont=dict(color='#333'), showgrid=False, linecolor='#ccc', showline=True),
                yaxis=dict(title='%', tickfont=dict(color='#333'), showgrid=True, gridcolor='#eee', range=[0,108]))
        else:
            # Comparação: Z1+Z2% por ano agrupado por modalidade
            _tipos = sorted(dh_ano['type'].unique())
            for _ai, _ano_c in enumerate(anos_sel):
                _dh_c = dh_ano[dh_ano['ano'] == _ano_c]
                _pt   = _dh_c.groupby('type')[['pb','pm','pa']].mean().reindex(_tipos).fillna(0)
                _cor_ano = _CORES_ANOS[_ai % len(_CORES_ANOS)]
                fig_hr_mod.add_trace(go.Bar(
                    x=_tipos, y=_pt['pb'].values,
                    name=f'{_ano_c} — Z1+Z2%', marker_color=_cor_ano, opacity=0.85,
                    text=[f"{v:.0f}%" for v in _pt['pb'].values], textposition='inside',
                    textfont=dict(color='white', size=10),
                    hovertemplate=f'{_ano_c} | %{{x}}<br>Z1+Z2: <b>%{{y:.1f}}%</b><extra></extra>'))
            fig_hr_mod.update_layout(**PW, barmode='group',
                title=dict(text=f'❤️ HR Z1+Z2% — {ano_sel}', font=dict(color='#222', size=13)),
                height=380, legend=LEGEND,
                xaxis=dict(tickfont=dict(color='#333'), showgrid=False),
                yaxis=dict(title='Z1+Z2 %', tickfont=dict(color='#333'), showgrid=True, gridcolor='#eee', range=[0,108]))
        with col1:
            st.plotly_chart(fig_hr_mod, use_container_width=True, config={'displayModeBar': False, 'responsive': True, 'scrollZoom': False})

        # HR Geral — pizza ou barras comparativas
        if not _modo_comparacao:
            ts_hr = {z: dh_ano[c].sum() for z, c in zip(CORES_HR.keys(), ['zb','zm','za'])}
            ...
            fig_hr_gen.update_layout(**PW,
                ...
                height=380, showlegend=True, legend=dict(font=dict(color='#111', size=11)))
        else:
            fig_hr_gen = go.Figure()
            _zonas_lbl = list(CORES_HR.keys())
            _z_cols    = ['zb','zm','za']
            for _ai, _ano_c in enumerate(anos_sel):
                _dh_c = dh_ano[dh_ano['ano'] == _ano_c]
                _tot  = _dh_c[_z_cols].sum(); _grand = _tot.sum()
                _pcts = (_tot / _grand * 100).values if _grand > 0 else [0,0,0]
                _cor_ano = _CORES_ANOS[_ai % len(_CORES_ANOS)]
                fig_hr_gen.add_trace(go.Bar(
                    x=_zonas_lbl, y=_pcts, name=str(_ano_c),
                    marker_color=_cor_ano, opacity=0.85,
                    text=[f"{v:.0f}%" for v in _pcts], textposition='inside',
                    textfont=dict(color='white', size=11),
                    hovertemplate=f'{_ano_c}<br>%{{x}}: <b>%{{y:.1f}}%</b><extra></extra>'))
            fig_hr_gen.update_layout(**PW, barmode='group',
                title=dict(text=f'❤️ HR Geral — {ano_sel}', font=dict(color='#222', size=13)),
                height=380, legend=LEGEND,
    ...
    if len(dr_ano) > 0:
        ...
        if not _modo_comparacao:
            piv_r = (dr_ano.groupby(['type','rz'], observed=True).size().unstack(fill_value=0))
            ...
            for zona, cor in CORES_RPE.items():
                ...
                fig_rpe_mod.add_trace(go.Bar(
                    x=pct_r['type'], y=vals, name=zona, marker_color=cor,
                    text=[f"{v:.0f}%" for v in vals], textposition='inside',
                    textfont=dict(color='white', size=11, family='Arial'),
                    hovertemplate=f'{zona}: <b>%{{y:.1f}}%</b><extra></extra>'))
            fig_rpe_mod.update_layout(**PW, barmode='stack',
                title=dict(text=f'🎯 RPE Zones por Modalidade — {ano_sel}', font=dict(color='#222', size=13)),
                height=380, legend=LEGEND,
                xaxis=dict(title='', tickfont=dict(color='#333'), showgrid=False, linecolor='#ccc', showline=True),
                yaxis=dict(title='%', tickfont=dict(color='#333'), showgrid=True, gridcolor='#eee', range=[0,108]))
        else:
            _tipos = sorted(dr_ano['type'].unique())
            _forte_col = 'Forte (7–10)'
            for _ai, _ano_c in enumerate(anos_sel):
                _dr_c = dr_ano[dr_ano['ano'] == _ano_c]
                if len(_dr_c) == 0: continue
                _piv = (_dr_c.groupby(['type','rz'], observed=True).size().unstack(fill_value=0))
                for z in CORES_RPE.keys():
                    if z not in _piv.columns: _piv[z] = 0
                _piv  = _piv.reindex(_tipos).fillna(0)
                _ptot = _piv.sum(axis=1).replace(0, np.nan)
                _pct  = (_piv.div(_ptot, axis=0) * 100).fillna(0)
                _cor_ano = _CORES_ANOS[_ai % len(_CORES_ANOS)]
                _vals_forte = _pct[_forte_col].values if _forte_col in _pct.columns else [0]*len(_tipos)
                fig_rpe_mod.add_trace(go.Bar(
                    x=_tipos, y=_vals_forte, name=f'{_ano_c} — Forte%',
                    marker_color=_cor_ano, opacity=0.85,
                    text=[f"{v:.0f}%" for v in _vals_forte], textposition='inside',
                    textfont=dict(color='white', size=10),
                    hovertemplate=f'{_ano_c} | %{{x}}<br>RPE Forte: <b>%{{y:.1f}}%</b><extra></extra>'))
            fig_rpe_mod.update_layout(**PW, barmode='group',
                title=dict(text=f'🎯 RPE Forte% — {ano_sel}', font=dict(color='#222', size=13)),
                height=380, legend=LEGEND,
                xaxis=dict(tickfont=dict(color='#333')),
                yaxis=dict(title='RPE Forte %', tickfont=dict(color='#333'), showgrid=True, gridcolor='#eee', range=[0,108]))
        with col3:
            st.plotly_chart(fig_rpe_mod, use_container_width=True, config={'displayModeBar': False, 'responsive': True, 'scrollZoom': False})

        if not _modo_comparacao:
            piv_r2 = (dr_ano.groupby(['type','rz'], observed=True).size().unstack(fill_value=0))
            for z in CORES_RPE.keys():
                if z not in piv_r2.columns: piv_r2[z] = 0
            ts_rpe = {z: piv_r2[z].sum() for z in CORES_RPE.keys()}
            ...
            fig_rpe_gen = go.Figure()
            _zonas_rpe = list(CORES_RPE.keys())
            for _ai, _ano_c in enumerate(anos_sel):
                _dr_c = dr_ano[dr_ano['ano'] == _ano_c]
                _piv  = (_dr_c.groupby('rz', observed=True).size().reindex(_zonas_rpe, fill_value=0))
                _tot  = _piv.sum()
                _pcts = (_piv / _tot * 100).values if _tot > 0 else [0,0,0]
                _cor_ano = _CORES_ANOS[_ai % len(_CORES_ANOS)]
                fig_rpe_gen.add_trace(go.Bar(
                    x=_zonas_rpe, y=_pcts, name=str(_ano_c),
                    marker_color=_cor_ano, opacity=0.85,
                    text=[f"{v:.0f}%" for v in _pcts], textposition='inside',
                    textfont=dict(color='white', size=11),
                    hovertemplate=f'{_ano_c}<br>%{{x}}: <b>%{{y:.1f}}%</b><extra></extra>'))
            fig_rpe_gen.update_layout(**PW, barmode='group',
                title=dict(text=f'🎯 RPE Geral — {ano_sel}', font=dict(color='#222', size=13)),
                height=380, legend=LEGEND,
                xaxis=dict(tickfont=dict(color='#333')),
                yaxis=dict(title='%', tickfont=dict(color='#333'), showgrid=True, gridcolor='#eee', range=[0,108]))
    ...
    for ano in sorted(df['ano'].unique(), reverse=True):
        ...
        for mod in mods_ano + ['Geral']:
            ...
            if len(sub_r) > 0:
                t = len(sub_r)
                for z, lbl in [('Leve (1–4)','RPE Z1%'),('Moderado (5–7)','RPE Z2%'),('Forte (7–10)','RPE Z3%')]:
    ...
    if rows_anual:
        ...
        for c in display_cols:
            html += f'<th style="border:1px solid #444;padding:7px 12px;text-align:center;font-weight:600">{c}</th>'
        html += '</tr>'
        for _, row in df_anual.iterrows():
            ...
            for c in display_cols:
                ...
                if c in zone_cols and not is_geral:
                    ...
                    html += f'<td style="border:1px solid #ddd;padding:5px 10px;text-align:center;background:{bg};font-weight:{fw}">{val}</td>'
                else:
                    html += f'<td style="border:1px solid #ddd;padding:5px 10px;text-align:center;background:{row_bg};color:#222;font-weight:{fw}">{val}</td>'
            html += '</tr>'
        html += '</table>'
    ...
    if zonas_hr and rpe_col:
        ...
        if len(dh_ok) > 0:
            ...
            st.caption("**Range de HR Zone (%) por nível de RPE** — boxplot por modalidade.")
            _n_mods = len(mods_disp)
            _fig_box = make_subplots(rows=1, cols=_n_mods,
                ...
                for hv, lbl, cor in HR_VARS:
                    ...
                    if y_vals:
                        _fig_box.add_trace(go.Box(
                            x=x_vals, y=y_vals, name=lbl, legendgroup=lbl,
                            marker_color=cor, line_color=cor,
                            showlegend=(_ci == 1), boxmean=False,
            ...
            _fig_box.update_layout(
                ...
                boxmode='group', margin=dict(l=30, r=10, t=40, b=40),
            ...
            st.plotly_chart(_fig_box, use_container_width=True, config={'displayModeBar': False, 'responsive': True})
            st.markdown("---")
            st.caption("**Correlação HR Zone × RPE Zone por modalidade e ano.** Força: Muito Forte ≥0.7 | Forte ≥0.5 | Moderada ≥0.3 | Fraca <0.3")
            _mod_tabs = st.tabs([f"🚴 {m}" if m=='Bike' else f"🚣 {m}" if m=='Row' else f"🎿 {m}" if m=='Ski' else f"🏃 {m}" if m=='Run' else m for m in mods_disp])
            _rpe_zone_labels = [
                ...
                with _mod_tabs[_ti]:
                    ...
                    for _ano_c in _anos_mod + ['Total']:
                        ...
                        for hv, hr_lbl, rpe_lbl in _rpe_zone_labels:
                            ...
                            emoji_f = ('🔴' if forca_v == 'Muito Forte' else '🟠' if forca_v == 'Forte' else '🟡' if forca_v == 'Moderada' else '🟢')
                            dir_v = '↗ RPE↑ → mais HR nesta zona' if r_v > 0 else '↘ RPE↑ → menos HR nesta zona'
                            _corr_rows.append({'Ano': str(_ano_c), 'HR Zone': hr_lbl, 'RPE Zone': rpe_lbl, 'Força': f"{emoji_f} {forca_v}", 'Direcção': dir_v})
                    if _corr_rows:
                        _df_corr = pd.DataFrame(_corr_rows)
                        try:
                            _pivot = _df_corr.pivot_table(index=['HR Zone','RPE Zone'], columns='Ano', values='Força', aggfunc='first').reset_index()
                        ...
                        st.caption("🔴 Muito Forte (r≥0.7) | 🟠 Forte (r≥0.5) | 🟡 Moderada (r≥0.3) | 🟢 Fraca (r<0.3).")
