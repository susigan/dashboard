...
def analisar_falta_estimulo(df_act_full, janela_dias=14, baseline_dias=90):
    """
    Need Score v4 por modalidade — componentes A/C/D via CTLγ (icu_training_load).

    Componentes base:
      A (25%) — Share CTLγ deficit vs baseline 90d   ← migrado de FTLM(dur×RPE) para CTLγ
      B (25%) — Quality deficit: carga_Z3/carga_total vs baseline  ← mantém RPE (único que precisa)
      C (20%) — Load deficit icu_training_load na janela vs típico  ← migrado para icu_training_load
      D (20%) — CTLγ slope_pct variação total no baseline  ← migrado para CTLγ slope
      E (10%) — Interacção A×B normalizada

    Porque CTLγ para A/C/D e RPE apenas para B:
      - CTLγ usa icu_training_load (potência real Bike/Row/Ski/Run) → consistente com PMC/FMT/NLSS
      - RPE×dur tinha variância alta e escala inconsistente entre modalidades
      - B precisa de RPE (RPE≥7 = proxy Z3) pois icu_training_load não separa intensidade
      - A/C/D medem quantidade de carga → CTLγ é mais preciso e consistente
    ...
    # load_rpe: mantido apenas para B (quality deficit via RPE≥7)
    df['load_rpe'] = df['dur_min'] * df['rpe_n']
    df['load_z3']  = np.where(df['rpe_n'] >= 7, df['load_rpe'], 0.0)

    # CTLγ via icu_training_load — base de A, C, D
    # Fallback para dur×RPE se icu_training_load não disponível
    if 'icu_training_load' in df.columns and df['icu_training_load'].notna().sum() > 10:
        df['load'] = pd.to_numeric(df['icu_training_load'], errors='coerce').fillna(0)
        _load_fonte = 'icu_training_load'
    else:
        df['load'] = df['load_rpe'].fillna(0)
        _load_fonte = 'dur×RPE (fallback)'

    hoje     = pd.Timestamp.now().normalize()
    ini_jan  = hoje - pd.Timedelta(days=janela_dias)
    ini_base = hoje - pd.Timedelta(days=baseline_dias)

    # ── CTLγ por modalidade (ewm span=28 de icu_training_load) ───────────────
    # Equivalente ao FTLM mas na escala correcta e consistente com o dashboard
    ...
    for mod in MODS:
        ...
        ld['CTLg'] = ld['load'].ewm(span=28, adjust=False).mean()
        ctlg_mods[mod] = ld.set_index('Data')

    ctlg_total = sum(ctlg_mods[m]['CTLg'] for m in MODS).replace(0, np.nan)
    ...
    for mod in MODS:
        ...
        # ── A — Share CTLγ deficit (migrado de FTLM para CTLγ) ───────────────
        share_jan  = float((fm.loc[ini_jan:hoje,  'CTLg'] /
                            ctlg_total.loc[ini_jan:hoje]).mean())
        share_base = float((fm.loc[ini_base:hoje, 'CTLg'] /
                            ctlg_total.loc[ini_base:hoje]).mean())
        ...
        # ── B — Quality deficit (mantém RPE — único componente que precisa) ──
        # RPE≥7 = proxy de sessão Z3; icu_training_load não distingue intensidade
        ...
        # ── C — Load deficit via icu_training_load (migrado de dur×RPE) ──────
        ...
        # ── D — CTLγ slope_pct (migrado de FTLM slope para CTLγ slope) ───────
        ctlg_base_series = fm.loc[ini_base:hoje, 'CTLg'].dropna()
        slope_pct = 0.0
        D = 0.0
        ftlm_ini = 0.0
        ftlm_fim = 0.0
        if len(ctlg_base_series) >= 10:
            ftlm_ini = float(ctlg_base_series.iloc[0])
            ftlm_fim = float(ctlg_base_series.iloc[-1])
        ...
        # Slope CTLγ na janela activa
        ftlm_jan_series = fm.loc[ini_jan:hoje, 'CTLg'].dropna()
        slope_pct_jan = 0.0
        fi = 0.0
        ff = 0.0
        ...
        # Guard 2: CTLγ pequeno — slope instável (início de época)
        _ftlm_p10 = float(fm['CTLg'].quantile(0.10)) if len(fm['CTLg'].dropna()) >= 10 else 0.0
        ...
        debug_rows.append({
            ...
            'D CTLg ini':            round(ftlm_ini if len(ctlg_base_series)>=10 else 0, 1),
            'D CTLg fim':            round(ftlm_fim if len(ctlg_base_series)>=10 else 0, 1),
...
# ══════════════════════════════════════════════════════════════════════════════
# MÁQUINA DE ESTADOS HRV-GUIDED — Javaloyes (LnRMSSD 7d) + Kiviniemi (HF power 10d)
# Fonte única partilhada por tab_recovery e tab_visao_geral (HIGH/LOW/REST).
# Réplica fiel da lógica da tab_recovery.
# ══════════════════════════════════════════════════════════════════════════════

def _state_machine_hrv(vals, sig_fn, max_high=2, max_train_days=None):
    """Máquina de estados fiel à Fig.1 (Kiviniemi/Javaloyes). Devolve (prescricoes, sinais)."""
