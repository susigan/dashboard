...
#
# Implementa o Algorithm 1 do paper:
#   - Prior AGMT2: K1=2.87, K2=4.09, T1=40.64d, T2=6.58d
#   - Penalidade Mahalanobis com Σ_pop da coorte de 30 atletas
#   - λ(n) adaptativo: prior domina com poucos testes, dados dominam com muitos
#   - Janela 90 dias, recalibra semanalmente
#   - Warm start L-BFGS-B com gradiente analítico
# ══════════════════════════════════════════════════════════════════════════════
...
# Matriz de correlação empírica da coorte AGMT2 (Tabela 5)
_NLSS_CORR = np.array([
    [ 1.00, +0.63, -0.41, -0.12],
    [+0.63,  1.00, +0.09, -0.28],
    [-0.41, +0.09,  1.00, +0.19],
...
_NLSS_SIGMA_INV = np.linalg.inv(_NLSS_SIGMA_POP)   # precision matrix (Cholesky-stable)

# ── B. Box constraints (Tabela 2) ─────────────────────────────────────────────
...
def _nlss_lambda(n_tests: int) -> float:
    """λ(n) = λ_max × exp(-n / n_half)"""
    return _NLSS_LAMBDA_MAX * np.exp(-n_tests / _NLSS_N_HALF)


# ── D. Performance prediction p̂(t) (Eq. 2) ───────────────────────────────────
def _nlss_predict(theta: np.ndarray, tss: np.ndarray, test_days: list,
                  p0: float = 0.0) -> np.ndarray:
    """
    p̂(t) = p0 + Σ_{τ<t} TSS(τ) × [K1×exp(-(t-τ)/T1) - K2×exp(-(t-τ)/T2)]
    """
    K1, K2, T1, T2 = theta
    ...
    for i, d in enumerate(test_days):
        ...
        for tau in range(d):
            delta = d - tau
            s += tss[tau] * (K1 * np.exp(-delta / T1) - K2 * np.exp(-delta / T2))
        p_hat[i] = p0 + s
    return p_hat


# ── E. Cost function + analytic gradient (Eqs. 5-10) ─────────────────────────
def _nlss_cost_grad(theta: np.ndarray, tss: np.ndarray, test_days: list,
                    test_watts: np.ndarray, lam: float,
                    p0: float = 0.0) -> tuple[float, np.ndarray]:
    """
    L(θ) = L_data + λ × L_prior  com gradiente analítico.
    L_data  = (1/n) Σ (p(d) - p̂(d;θ))²
    L_prior = (θ - μ_pop)ᵀ Σ⁻¹_pop (θ - μ_pop)
    """
    K1, K2, T1, T2 = theta
    n = len(test_days)
    ...
    for i, d in enumerate(test_days):
        ...
        for tau in range(d):
            ...
            se1  += w * e1
            se2  += w * e2
            ste1 += w * (delta / (T1 * T1)) * e1
            ste2 += w * (delta / (T2 * T2)) * e2
    ...
    for i, r in enumerate(residuals):
        g_data[0] += -2.0 / n * r * cache_e1[i]   # ∂L/∂K1
        g_data[1] += +2.0 / n * r * cache_e2[i]   # ∂L/∂K2
        g_data[2] += -2.0 * K1 / n * r * cache_te1[i]  # ∂L/∂T1
        g_data[3] += +2.0 * K2 / n * r * cache_te2[i]  # ∂L/∂T2
...
def calcular_nlss(df_act, df_wellness=None,
    ...
    INPUT:
        df_act      : actividades completas (ac_full) — precisa de
                      icu_training_load e mmp20_pr_w / mmp20_is_pr
        df_wellness : não usado directamente mas reservado para futuro

    OUTPUT (dict):
        theta       : [K1, K2, T1, T2] mais recentes
        K1, K2, T1, T2 : floats individualizados
        lambda_n    : λ final (quão dominante é o prior)
        n_tests     : número de testes na janela
        p_hat_series: série p̂(t) sobre toda a história (pd.Series, index=Data)
        CTL_nlss    : série CTL com K1 corrigido (pd.Series)
        ATL_nlss    : série ATL com K2 corrigido (pd.Series)
        TSB_nlss    : CTL_nlss - ATL_nlss (pd.Series)
        CTL_tp      : CTL TrainingPeaks (K1=K2=1, span 42/7) para comparação
        ATL_tp      : ATL TrainingPeaks para comparação
        TSB_tp      : TSB TrainingPeaks para comparação
        test_dates  : lista de datas dos testes usados
        test_watts  : lista de potências dos testes usados
        history     : lista de dicts semanais {day, K1, K2, T1, T2, lambda, n}
        fonte_carga : 'icu_training_load' ou 'session_rpe'
        error       : str ou None
    """
    from scipy.optimize import minimize as _minimize

    # ── 1. Preparar série diária de TSS ──────────────────────────────────────
    df = filtrar_principais(df_act).copy()
    df['Data'] = pd.to_datetime(df['Data'])
    ...
    if 'icu_training_load' in df.columns and df['icu_training_load'].notna().sum() > 10:
        ...
        return {'error': 'Sem dados de carga (icu_training_load ou RPE)', 'theta': _NLSS_MU_POP}

    ld = df.groupby('Data')['_load'].sum().reset_index().sort_values('Data')
    date_range = pd.date_range(ld['Data'].min(), datetime.now().date())
    ld = ld.set_index('Data').reindex(date_range, fill_value=0).reset_index()
    ld.columns = ['Data', 'tss_val']
    tss_arr = ld['tss_val'].values.astype(float)
    n_days  = len(tss_arr)

    # ── 2. Testes de potência — mmp20_pr_w onde is_pr=True ───────────────────
    # mmp20_pr_w: watts só quando a sessão estabeleceu um PR (data exacta conhecida)
    # Fallback: mmp5_pr_w se mmp20 insuficiente
    test_records = []
    for _mmp_key, _dur_label in [('mmp20_pr_w', '20min'), ('mmp5_pr_w', '5min'),
                                   ('mmp12_pr_w', '12min'), ('mmp3_pr_w', '3min')]:
        if _mmp_key in df.columns:
            _pr = df[[' Data', _mmp_key]].dropna(subset=[_mmp_key]) if ' Data' in df.columns \
                  else df[['Data', _mmp_key]].dropna(subset=[_mmp_key])
            _pr.columns = ['Data', 'watts']
            _pr = _pr[_pr['watts'] > 50].copy()
            _pr['dur'] = _dur_label
            test_records.append(_pr)

    # Corrigir nome de coluna Data
    for _mmp_key in ['mmp20_pr_w', 'mmp5_pr_w', 'mmp12_pr_w', 'mmp3_pr_w']:
        if _mmp_key in df.columns:
            _pr = df[['Data', _mmp_key]].dropna(subset=[_mmp_key])
            _pr.columns = ['Data', 'watts']
            _pr = _pr[_pr['watts'] > 50].copy()
            _pr['dur'] = _mmp_key
            test_records.append(_pr)
    ...
    if test_records:
        ...
        # Para cada data, usar o MMP mais longo disponível (mais estável)
        tests_df = tests_df.sort_values(['Data', 'dur']).drop_duplicates('Data', keep='last')
        tests_df = tests_df.sort_values('Data')
    else:
        tests_df = pd.DataFrame(columns=['Data', 'watts', 'dur'])

    # Mapear datas de teste para índice na série diária
    date_to_idx = {pd.Timestamp(d).date(): i for i, d in enumerate(ld['Data'])}
    test_days   = []
    test_watts  = []
    test_dates  = []
    for _, row in tests_df.iterrows():
        ...
        if d in date_to_idx:
            idx = date_to_idx[d]
            if idx > 0:  # precisa de historial antes do teste
                test_days.append(idx)
                test_watts.append(float(row['watts']))
                test_dates.append(d)

    # ── 3. p0 — baseline de performance ──────────────────────────────────────
    p0 = float(np.nanpercentile(test_watts, p0_frac * 100)) if test_watts else 200.0

    # ── 4. Weekly recalibration loop (Algorithm 1) ───────────────────────────
    theta_current = _NLSS_MU_POP.copy()
    history       = []
    DELTA_REL_THR = 0.15

    for t in range(6, n_days, 7):   # cada 7 dias
        w_start = max(0, t - window_l)

        # Testes na janela [t-L, t]
        w_test_days  = [d for d in test_days  if w_start <= d <= t]
        w_test_watts = [test_watts[test_days.index(d)] for d in w_test_days]
        n_t          = len(w_test_days)
        ...
        # Optimizar com L-BFGS-B + warm start
        try:
            result = _minimize(
                lambda th: _nlss_cost_grad(th, tss_arr, w_test_days,
                                           np.array(w_test_watts), lam, p0),
                theta_current,
                method='L-BFGS-B',
                jac=True,
                bounds=_NLSS_BOUNDS,
            ...
            # Convergência + detecção regime change
            d_rel = (np.linalg.norm(theta_new - theta_current) /
                     (np.linalg.norm(theta_current) + 1e-12))
            if d_rel > DELTA_REL_THR:
                lam = _nlss_lambda(0)   # regime change → reset λ
            if np.any(np.abs(theta_new - _NLSS_MU_POP) > 3 * _NLSS_SD_POP):
                theta_new = theta_current  # rejeitar: fora de 3σ
            else:
                theta_current = theta_new.copy()
        except Exception:
            pass  # manter theta_current

        history.append({'day': t, 'n_tests': n_t, 'lambda': float(lam),
    ...
    lam_final = _nlss_lambda(len([d for d in test_days
                                   if (n_days - window_l) <= d <= n_days]))

    # ── 5. Série p̂(t) completa ───────────────────────────────────────────────
    # Calcula p̂ para todos os dias (não só dias de teste)
    p_hat_vals = np.full(n_days, np.nan)
    for t_day in range(1, n_days):
        s = 0.0
        for tau in range(t_day):
            delta = t_day - tau
            s += tss_arr[tau] * (K1 * np.exp(-delta / T1) - K2 * np.exp(-delta / T2))
        p_hat_vals[t_day] = p0 + s
    p_hat_series = pd.Series(p_hat_vals, index=pd.DatetimeIndex(ld['Data']), name='p_hat')

    # ── 6. CTL_nlss / ATL_nlss — com K1/K2 individualizados ─────────────────
    # CTL_nlss = K1 × EWM(span=T1)  |  ATL_nlss = K2 × EWM(span=T2)
    # Equivalente ao modelo Banister com parâmetros individualizados
    # Nota: EWM(span=T) ≈ decaimento exponencial com τ = T/2 (alpha = 2/(T+1))
    tss_series  = pd.Series(tss_arr, index=pd.DatetimeIndex(ld['Data']))
    ctl_nlss    = K1 * tss_series.ewm(span=T1, adjust=False).mean()
    atl_nlss    = K2 * tss_series.ewm(span=T2, adjust=False).mean()
    tsb_nlss    = ctl_nlss - atl_nlss

    # TrainingPeaks para comparação (K1=K2=1, T1=42, T2=7)
